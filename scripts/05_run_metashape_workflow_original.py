...
CONFIG_PATH = REPO_ROOT / "workflow" / "automation_workflow_original.md"
...
def build_depth_maps_compatible(chunk, dense_config):
    ...
    try:
        ...
        # En el bloque original, buildDenseCloud puede aprovechar esos datos aunque
        # chunk.depth_maps no quede expuesto por la API.
        print("buildDepthMaps lanzo una excepcion.")
        print("Detalle:", exc)
        print("Se intentara construir Dense Cloud de todos modos.")
        return False
...
def build_dense_cloud_compatible(chunk, dense_config):
    ...
    if has_depth_maps(chunk):
        ...
        print(
            "No se detectan depth maps por API; "
            "se intentara buildDenseCloud de todos modos."
        )
